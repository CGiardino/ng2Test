"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var BasketComponent = (function () {
    function BasketComponent() {
        this.title = "Basket";
        this.selectedProducts = null;
        this.checkoutProducts = new core_1.EventEmitter();
    }
    BasketComponent.prototype.onChange = function ($event, isConfirmed) {
        this.checkoutProducts.emit({ confirmed: isConfirmed });
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Array)
    ], BasketComponent.prototype, "selectedProducts", void 0);
    __decorate([
        core_1.Output(), 
        __metadata('design:type', Object)
    ], BasketComponent.prototype, "checkoutProducts", void 0);
    BasketComponent = __decorate([
        core_1.Component({
            selector: 'basket',
            template: "\n        <h2>{{ title }}</h2>\n        <li *ngFor=\"let product of selectedProducts\">\n            <span id=\"basketProduct{{ product.key }}\">\n                {{ product.value }}\n            </span>\n        </li>\n        <button *ngIf=\"selectedProducts!=null && selectedProducts!=''\" \n            id=\"checkout\"\n            type=\"button\"  \n            (click)=\"onChange($event, sport)\">Checkout\n        </button>\n    "
        }), 
        __metadata('design:paramtypes', [])
    ], BasketComponent);
    return BasketComponent;
}());
exports.BasketComponent = BasketComponent;
//# sourceMappingURL=basket.component.js.map